...
public class TypesAdapter extends RecyclerView.Adapter<StatItemHolder> {

    private Context context;
    private ArrayList<Type> types;
    private ArrayList<RelationCardFormat> relations;

    public TypesAdapter(@NonNull Context context, @NonNull ArrayList<Type> objects, ArrayList<RelationCardFormat> relations) {
    ...
    public void onBindViewHolder(StatItemHolder holder, int position) {
        ...
        holder.titleClass.setText(type.get_name());

        //Calcul de la statistique pour le type courant
        int nb = 0;
        int nbTot = 0;
        for (RelationCardFormat relation : relations){
            if (type.get_id() == relation.get_card().get_type().get_id()) {
                nbTot++;
